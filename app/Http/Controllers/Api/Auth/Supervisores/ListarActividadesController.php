...
class ListarActividadesController extends Controller
{
    ...
    foreach($actividades as $ac){
        $fe = DB::table($ac->nombre_tabla. ' as ac')
         ->select('ac.id','ac.fecha_inicio', 'ac.fecha_fin', 'ac.id_plan_trabajo')
        ...
        foreach($fe as $fecha){
            if($fecha->id_plan_trabajo == $ac->id_plan_trabajo && $fecha->fecha_inicio <= $semanal.' 23:59:00' && date('Y-m-d').' 00:00:00' >= $fecha->fecha_inicio){
                $fecha->nombre_actividad = $ac->nombre_actividad.'-'.$fecha->id;
