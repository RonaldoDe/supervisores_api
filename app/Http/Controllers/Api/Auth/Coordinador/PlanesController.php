...
class PlanesController extends Controller
{
    ...
    public function oneActividad(Request $request)
    {
        $validator=\Validator::make($request->all(),[
            'id_plan_trabajo' => 'required',
        ...
        else
        {
            ...
            if($coordinador != null){
                ...
                foreach($actividades as $ac){
                    $fe = DB::table($ac->nombre_tabla. ' as ac')
                    ->where('ac.id_plan_trabajo',$ac->id_plan_trabajo)
                    ->where('ac.id',request('id_actividad'))
                    ->get();
                    ...
                    // validar si el array tiene actividades
                    return response()->json(['Actividad' => $lista_actividades_arr, 'Nombre' => $plan->nombre, 'Sucursal' => $plan->id_sucursal],200);
