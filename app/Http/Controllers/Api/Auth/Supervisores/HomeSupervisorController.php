...
/*Author Ronaldo camacho */
    ...
    public function index(Request $request)
    {
       ...
       foreach($actividades as $ac){
            ...
            ->whereIn('ac.id_estado',[1, 3])
            ->get();

            //obtener las actividades asiganadas a la fecha actual de un supervisor
            foreach($fe as $fecha){
                if((date('Y-m-d').' 00:00:00' >= $fecha->fecha_inicio && date('Y-m-d').' 00:00:00' <= $fecha->fecha_fin && $fecha->id_plan_trabajo == $ac->id_plan_trabajo) || ($fecha->id_plan_trabajo == $ac->id_plan_trabajo && $fecha->id_estado == 3)){
